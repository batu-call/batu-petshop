        ...
        {features && features.length > 0 ? (
          <ul className="list-disc list-inside space-y-3">
            {features.map((feature, index) => (
              <li key={index} className="text-color dark:text-[#0b8457]!">
                <strong className="text-color dark:text-[#0b8457]! opacity-90 font-semibold mb-2 text-shadow-2xs">
                  {feature.name}
                </strong>
                <p className="text-color dark:text-[#0b8457]!">{feature.description}</p>
              </li>
            ))}
          </ul>
        ) : (
