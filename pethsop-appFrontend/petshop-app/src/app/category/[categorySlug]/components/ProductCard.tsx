...
const ProductCard = ({
  ...
  const shouldPrioritize = isFirstPage && index < 4;
    ...
    <div className="bg-primary w-full sm:w-auto rounded-2xl shadow-md hover:shadow-xl flex flex-col overflow-hidden justify-between transition duration-300 ease-in-out hover:scale-[1.02] relative group">
      ...
      >
        <Heart
          className={`w-3 h-3 transition-colors duration-300 ${
            isFavorite(p._id) ? "text-gray-600 dark:!text-black" : "text-gray-400 dark:!text-black"
          }`}
      ...
      >
        <Heart
          ...
          className={`w-3.5 h-3.5 transition-all duration-300 ${isFavorite(p._id) ? "scale-110" : ""}`}
