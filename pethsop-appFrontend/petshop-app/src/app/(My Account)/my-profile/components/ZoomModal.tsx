          ...
          >
            ...
            >
              <div className="relative w-full h-full">
                <Image
                  src={currentUrl} alt={label} fill sizes="(max-width: 635px) 88vw, 820px" priority
                  className="object-contain"
                  style={{ opacity: imgLoaded ? 1 : 0, transition: "opacity 0.22s ease", filter: "drop-shadow(0 10px 36px rgba(0,0,0,0.65))" }}
                  onLoad={(e) => { const img = e.currentTarget; setNaturalSize({ w: img.naturalWidth, h: img.naturalHeight }); setImgLoaded(true); }}
                />
              </div>
            </div>
            ...
            <div className="flex-shrink-0 px-3 py-2.5" style={{ borderTop: "1px solid rgba(255,255,255,0.055)" }}>
              <div ref={thumbnailRef} className="zm-thumb-bar flex gap-2 overflow-x-auto justify-center">
                {images.map((url, index) => (
                  <button key={index} onClick={() => goToIndex(index)} className="relative flex-shrink-0 overflow-hidden transition-all duration-200 hover:opacity-80"
                    style={{ width: 50, height: 50, borderRadius: 9, border: currentIndex === index ? "2px solid #97cba9" : "2px solid rgba(255,255,255,0.07)", opacity: currentIndex === index ? 1 : 0.42, background: "#161a17", transform: currentIndex === index ? "scale(1.08)" : "scale(1)", outline: "none", cursor: "pointer", boxShadow: currentIndex === index ? "0 0 0 3px rgba(151,203,169,0.15)" : "none" }}>
          ...
          <div className="flex gap-1.5 pointer-events-auto">
            {images.map((_, i) => (
              <button key={i} onClick={() => goToIndex(i)}
                style={{ width: i === currentIndex ? 20 : 6, height: 6, borderRadius: 3, background: i === currentIndex ? "#97cba9" : "rgba(255,255,255,0.18)", border: "none", cursor: "pointer", padding: 0, transition: "width 0.25s ease, background 0.25s ease" }}
              />
            ))}
          </div>
        )}
