...
const parseMessage = (
  ...
  if (
    ...
    return {
      text,
      products: [],
      orderSummary: parsed as OrderSummary,
      cart: null,
    };
  }
          ...
          <Link
            ...
            className={`inline-flex items-center gap-0.5 font-semibold underline underline-offset-2 transition-colors ${
              isUser
                ...
                : "text-[#0b8457] dark:text-[#57B394] hover:text-[#393E46] dark:hover:text-[#97cba9]"
            }`}
          >
            {linkMatch[1]}
        ...
        result.push(
          <strong
            ...
            className={
              isUser
                ? "font-bold"
                : "font-bold text-[#393E46] dark:text-[#c8e6d0]"
            }
          >
            {seg.slice(2, -2)}
          </strong>,
      ...
      {lines.map((line, i) => {
        if (line.toLowerCase().startsWith("tip:") || line.includes("💡"))
          return (
            <div
              key={i}
              className="flex items-start gap-2 bg-amber-50 dark:bg-amber-900/20 border border-amber-100 dark:border-amber-700/30 rounded-xl px-2.5 py-2 mt-1"
            >
        ...
        if (isBullet)
          return (
            <div key={i} className="flex items-start gap-2.5">
              <span
                className={`mt-[7px] w-1.5 h-1.5 rounded-full flex-shrink-0 ${isUser ? "bg-white/60" : "bg-[#97cba9] dark:bg-[#57B394]"}`}
              />
              <span
                className={
                  isUser
                    ? "text-white/95"
                    : "text-gray-700 dark:text-[#c8e6d0]/90"
                }
              >
        ...
        if (numMatch)
          return (
            <div key={i} className="flex items-start gap-2.5">
              <span
                className={`flex-shrink-0 w-[18px] h-[18px] rounded-full text-[9px] font-black flex items-center justify-center mt-0.5 ${isUser ? "bg-white/20 text-white" : "bg-[#393E46]/10 dark:bg-[#c8e6d0]/10 text-[#393E46] dark:text-[#c8e6d0]"}`}
              >
                {numMatch[1]}
              </span>
              <span
        ...
        return (
          <p
            key={i}
            className={
              isUser ? "text-white/95" : "text-gray-700 dark:text-[#c8e6d0]/90"
            }
          >
            {renderInline(line)}
          </p>
    ...
    <Link href={`/Products/${product.slug}`} onClick={onNavigate}>
      <div className="group flex items-center gap-3 bg-white dark:bg-[#1e3d2a] border border-gray-100 dark:border-white/8 rounded-2xl p-2.5 hover:border-[#97cba9] dark:hover:border-[#57B394]/50 hover:shadow-md transition-all duration-200 cursor-pointer">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-xs font-semibold text-gray-800 dark:text-[#c8e6d0] truncate">
            {product.product_name}
          </p>
          <div className="flex flex-wrap items-center gap-1 mt-0.5">
            <span className="text-[10px] text-gray-400 dark:text-[#7aab8a] bg-gray-100 dark:bg-[#162820] px-1.5 py-0.5 rounded-full">
              {product.category}
            </span>
            {product.avgRating && (
            ...
            {isLowStock && (
              <span className="text-[10px] text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/20 px-1.5 py-0.5 rounded-full">
          ...
          <div className="flex items-center gap-2 mt-1">
            {product.salePrice && product.salePrice < product.price ? (
              <>
                ...
                <span className="text-sm font-bold text-[#393E46] dark:text-[#c8e6d0]">
        ...
        <div className="flex-shrink-0 w-7 h-7 rounded-xl bg-gray-50 dark:bg-[#162820] group-hover:bg-[#393E46] dark:group-hover:bg-[#57B394]/20 flex items-center justify-center transition-colors duration-200">
          <ChevronRight
            size={13}
            className="text-gray-400 dark:text-[#7aab8a] group-hover:text-white transition-colors duration-200"
          />
        </div>
      </div>
    </Link>
...
const OrderSummaryCard = ({ summary }: { summary: OrderSummary }) => {
  const active = statusRows.filter(
    (s) => (summary[s.key as keyof OrderSummary] as number) > 0,
  );
  return (
    <div className="bg-white dark:bg-[#1e3d2a] border border-gray-200 dark:border-white/8 rounded-2xl overflow-hidden">
      ...
      <div className="bg-gradient-to-r from-gray-50 dark:from-[#162820] to-white dark:to-[#1e3d2a] px-3.5 py-2.5 flex items-center justify-between border-b border-gray-100 dark:border-white/5">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[10px] text-gray-500 dark:text-[#7aab8a] font-medium">
            Order History
          </span>
        </div>
      ...
      <div className="p-3 space-y-3">
        ...
        {active.length > 0 && (
          <div className="grid grid-cols-2 gap-1.5">
            {active.map((s) => (
              <div
                key={s.key}
                className="flex items-center gap-2 bg-gray-50 dark:bg-[#162820] rounded-xl px-2.5 py-2"
              >
                <span
                  className={`w-2 h-2 rounded-full flex-shrink-0 ${s.dot}`}
                />
                <div>
                  <p className={`text-xs font-bold leading-none ${s.text}`}>
                    {summary[s.key as keyof OrderSummary]}
                  </p>
                  <p className="text-[9px] text-gray-400 dark:text-[#7aab8a] mt-0.5">
                    {s.label}
                  </p>
                </div>
              </div>
        ...
        <Link href="/my-orders">
          <button className="w-full flex items-center justify-center gap-2 bg-[#393E46] dark:bg-[#0E5F44] hover:bg-[#2a2f36] dark:hover:bg-[#57B394]/80 text-white text-xs font-semibold px-4 py-2.5 rounded-xl transition-all duration-200 group">
            ...
            <ExternalLink
              size={10}
              className="opacity-50 group-hover:opacity-100 transition-opacity ml-auto"
            />
          </button>
        </Link>
      </div>
    ...
    <div className="group bg-white dark:bg-[#1e3d2a] border border-gray-200 dark:border-white/8 rounded-2xl overflow-hidden hover:shadow-lg hover:border-[#97cba9]/40 dark:hover:border-[#57B394]/40 transition-all duration-200 cursor-pointer">
      ...
      <div className="p-3">
        <div className="space-y-2 mb-3">
            ...
            return (
              <div key={i} className="flex items-center gap-2.5">
                <img
                  ...
                  className="w-9 h-9 object-cover rounded-xl flex-shrink-0"
                />
                <div className="flex-1 min-w-0">
                  <p className="text-[11px] font-medium text-gray-700 dark:text-[#c8e6d0] truncate">
                    {item.product?.product_name}
                  </p>
                  <p className="text-[10px] text-gray-400 dark:text-[#7aab8a]">
                    Qty: {item.quantity}
                  </p>
                </div>
                <div className="text-right flex-shrink-0">
                  ...
                  <p className="text-[11px] font-bold text-[#393E46] dark:text-[#c8e6d0]">
          ...
          {cart.items.length > 3 && (
            <p className="text-[10px] text-gray-400 dark:text-[#7aab8a] pl-11">
        ...
        <div className="flex items-center justify-between pt-2 border-t border-gray-50 dark:border-white/5">
          <span className="text-[10px] text-gray-400 dark:text-[#7aab8a]">
            Subtotal
          </span>
          <div className="flex items-center gap-2">
            ...
            <div className="w-6 h-6 rounded-lg bg-gray-50 dark:bg-[#162820] group-hover:bg-[#97cba9] dark:group-hover:bg-[#0E5F44] flex items-center justify-center transition-colors duration-200">
              <ChevronRight
                size={11}
                className="text-gray-400 dark:text-[#7aab8a] group-hover:text-white transition-colors"
              />
            </div>
          </div>
        </div>
...
const MessageBubble = ({
  ...
  return (
    <SlideIn isUser={isUser}>
      ...
      >
        {isUser ? (
          <UserAvatar avatarUrl={user?.avatar ?? null} />
        ) : (
          <BotAvatar />
        )}
        <div
        ...
        >
          {msg.content && (
            <div
              className={`px-3.5 py-2.5 rounded-2xl shadow-sm ${isUser ? "bg-[#393E46] dark:bg-[#0E5F44] rounded-br-sm" : "bg-white dark:bg-[#1e3d2a] border border-gray-100 dark:border-white/8 rounded-bl-sm"}`}
            >
          ...
          {msg.orderSummary && (
            <div className="w-full">
              <OrderSummaryCard summary={msg.orderSummary} />
            </div>
          )}
          {msg.cart && (
            <div className="w-full flex flex-col gap-2">
          ...
          <span className="text-[9px] text-gray-300 dark:text-[#7aab8a]/50 px-1">
            {timeStr}
          </span>
        </div>
      </div>
...
const CustomerChatWidget = () => {
  ...
  useEffect(() => {
    ...
    if (!prev.includes("maximum-scale"))
      meta.content = prev + ", maximum-scale=1, user-scalable=no";
    return () => {
      meta.content = prev;
    };
  }, []);
            ...
            <div className="flex items-center gap-3">
              <div className="relative">
                <div className="w-9 h-9 rounded-2xl bg-[#D6EED6] dark:bg-[#0E5F44] flex items-center justify-center">
                  <Bot
                    size={18}
                    className="text-[#393E46] dark:text-[#c8e6d0]"
                  />
                </div>
              ...
              <div>
                ...
                <p className="text-gray-400 dark:text-[#7aab8a] text-[11px]">
                  Always here to help 🐾
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              {messages.length > 1 && (
                ...
                >
                  <Trash2
                    size={14}
                    className="text-white/60 group-hover:text-red-300 transition-colors"
                  />
                </button>
              )}
              <button
